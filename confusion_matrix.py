...
def main():  # Build and save confusion matrix
    ...
    for file in os.listdir(happiness_test_folder):
        l = LabelledText.from_text_file(os.path.join(happiness_test_folder, file), [happiness_vocabulary, sadness_vocabulary], cleaning_level=HIGH, fast=True)
        if 'happiness' == l.get_label():
            data['happiness']['happiness'] += 1
        else:
            data['happiness']['sadness'] += 1
    for file in os.listdir(sadness_test_folder):
        l = LabelledText.from_text_file(os.path.join(sadness_test_folder, file), [happiness_vocabulary, sadness_vocabulary], cleaning_level=HIGH, fast=True)
        if 'sadness' == l.get_label():
            data['sadness']['sadness'] += 1
        else:
            data['sadness']['happiness'] += 1
